...
- **검증**: 접근성(WCAG 2.2 AA), 반응형 디자인, Core Web Vitals, 성능 예산 자동 체크
- **테스트**: Playwright 브라우저 테스트 및 자동화 접근성 스크립트
- **자동 수정**: 접근성, 반응형, 성능 문제를 묻지 않고 바로 수정
...
| Phase | 이름 | 실행 시기 |
|-------|------|-----------|
| 1 | 분석 | 항상 — 프로젝트 컨텍스트, 작업 규모, 기존 컴포넌트, 디자인 방향성 |
| 2 | 설계 | 규모에 따라 — 디자인 씽킹, 컴포넌트 아키텍처, 스타일 전략, 구현 계획 |
| 3 | 구현 | 항상 — 컴포넌트 생성, 스타일링, 상태 관리, 디자인-투-코드, 에러 처리 |
| 4 | 검증 | 항상 — WCAG 2.2 AA 접근성, 반응형, Core Web Vitals, 브라우저 테스트, 자동 수정 |

### v2.0 하이라이트
- **디자인 미학**: 11가지 미학 방향성, Anti-AI-slop 규칙, 타이포그래피/색상/모션/공간 가이드라인
- **컴포넌트 아키텍처**: 설정보다 조합, 파일 콜로케이션, 상태 관리 결정 트리
- **성능**: Core Web Vitals 목표 (LCP ≤2.5s, INP ≤200ms, CLS ≤0.1), 성능 예산, 안티패턴 감지
- **접근성**: WCAG 2.2 AA 준수, 포커스/리플로우/간격/터치타겟 자동 Playwright 테스트 스크립트
- **브라우저 테스트**: Playwright 기반 정찰-후-액션 패턴
