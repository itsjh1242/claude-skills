...
Remind는 "이전 로그의 미완료 검색"이 아니라 **현재 작업 맥락에서 내일 이어할 것들을 정리**하는 것이다.

다음 소스를 **모두** 분석하여 Remind 항목을 자동 생성한다:

1. **현재 세션의 대화 맥락** (가장 중요):
   - 오늘 논의했지만 아직 완료되지 않은 작업
   - 다른 팀/사람의 응답을 기다리는 것 (예: BE 응답 대기, PR 리뷰 대기)
   - 작업 중 발견했지만 아직 처리하지 않은 것 (예: 미사용 코드 정리, 리팩토링 필요)
   - 다음 단계로 진행해야 할 것
2. **uncommitted 변경사항**: `git status`에서 WIP 감지
3. **이전 로그 carry-over**: tasks 레포에서 이전 미완료 항목 확인 (보조적)

자동 생성한 항목을 사용자에게 보여주고 확인받는다:
